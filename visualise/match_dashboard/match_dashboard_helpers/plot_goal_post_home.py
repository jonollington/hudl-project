...
def plot_goal_post_home(ax, shots_df, pitch2, line_color, home_color):
    
    home_team_id = shots_df['home_team_id'].iloc[1]
    home_team_name = shots_df[shots_df['team_id'] == home_team_id]['name'].iloc[0]
    home_shot_df = shots_df[(shots_df['team_id'] == home_team_id) & (shots_df['own_goal'] == False)].copy()
    
    pitch2.draw(ax=ax)
    line_width = 6

    # Filter the DataFrame for goals
    goals_df = home_shot_df[home_shot_df['type_display_name'] == 'Goal'].copy()

    # Filter the DataFrame for non-goals (shots)
    shots_df = home_shot_df[(home_shot_df['type_display_name'] != 'Goal') & (home_shot_df['blocked_shot'] != True) & (home_shot_df['type_display_name'] != 'MissedShots') & (home_shot_df['type_display_name'] != 'ShotOnPost')].copy()
    ...
    pitch2.scatter(x_shots, y_shots, marker='o', color=home_color, ec='w',zorder=3, s=350, ax=ax, alpha=0.8)
    ...
    home_shot_df[
        ...
        (home_shot_df['blocked_shot'] != True)
    ])

    # Define the bounding box properties
    home_shots_bbox = dict(boxstyle="round,pad=0.3", facecolor=home_color)

    # Place the text without a bounding box
    ax.text(100, 90, f"{home_team_name} shots on target", color='silver', fontsize=20, ha='left')

    # Place the number with a bounding box right after the text
    ax.text(0, 90, f"{shots_count}", color='w', fontsize=20, ha='right', weight='bold', bbox=home_shots_bbox)
