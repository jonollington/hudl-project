...
def get_shot_events(match_id: int) -> pd.DataFrame:
    ...
    {{
      live_match_event(
        where: {{ match_id: {{ _eq: {match_id} }}, name: {{ _eq: "shot" }} }}
      ) {{
        name
        outcome
        player_id
    ...
    # Send requests
    r1 = requests.post(graphql_url, json={"query": query_shots}, headers=headers)
